...
describe('packageSlice', () => {
  describe('fetchPackages', () => {
    it('should fetch packages and update state correctly', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://127.0.0.1:3000/packages');

      expect(packageReducer(initialState, fetchPackages.fulfilled(fakePackages))).toEqual({
        loading: false,
    ...
    it('should handle errors correctly', async () => {
      ...
      expect(packageReducer(initialState, fetchPackages.rejected(fakeError))).toEqual({
        loading: false,
        flightpackage: [],
